...
#define uipMAC_ADDR0	emacETHADDR0
#define uipMAC_ADDR1	emacETHADDR1
#define uipMAC_ADDR2	emacETHADDR2
#define uipMAC_ADDR3	emacETHADDR3
#define uipMAC_ADDR4	emacETHADDR4
#define uipMAC_ADDR5	emacETHADDR5
...
extern void ( vEMAC_ISR_Wrapper )( void );
    ...
    {
        vParTestSetLED(3, 1); // Indicate error
...
void printHex(char id, uint32_t hex)
{
    static portCHAR cMessageForDisplay[ 32 ];
    extern xQueueHandle xLCDQueue;
    xLCDMessage xLCDMsg;

    cMessageForDisplay[0] = id;
    cMessageForDisplay[1] = ':';
    cMessageForDisplay[2] = _toHex(hex>>28);
    cMessageForDisplay[3] = _toHex(hex>>24);
    cMessageForDisplay[4] = _toHex(hex>>20);
    cMessageForDisplay[5] = _toHex(hex>>16);
    cMessageForDisplay[6] = _toHex(hex>>12);
    cMessageForDisplay[7] = _toHex(hex>>8);
    cMessageForDisplay[8] = _toHex(hex>>4);
    cMessageForDisplay[9] = _toHex(hex>>0);
    cMessageForDisplay[10] = ':';
    cMessageForDisplay[11] = _toHex(printCnt>>4);
    cMessageForDisplay[12] = _toHex(printCnt>>0);
    cMessageForDisplay[13] = 0;
    printCnt++;

    xLCDMsg.xColumn = 0;
    xLCDMsg.pcMessage = cMessageForDisplay;
    xQueueSend( xLCDQueue, &xLCDMsg, portMAX_DELAY );
}
