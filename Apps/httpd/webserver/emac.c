...
// Initializes the EMAC ethernet controller
    ...
    */
    ...
    MAC_MCFG = MCFG_HCLK_DIV_28; // Board specified
    ...
    for (tout = 100; tout; tout--); // Dummy delay for KS8xxx
    MAC_SUPP = 0;
    ...
    if (((id1 << 16) | (id2 & 0xFFF0)) == DP83848C_ID) {
        model = 1;
    } else if (((id1 << 16) | (id2 & 0xFFF0)) == KS8721B_ID) {
        model = 2;
    }
    if(model) {
    ...
    if( xReturn == pdPASS )
    {
        if(model == 1) { // DP83848C
            ...
            regv = read_PHY (0x1f); // Read KS8721B specified register. Stupid magic
            regv = (regv>>2)&7;
            if ((regv==5) || (regv==6)) {
                /* Full duplex is enabled. */
                MAC_MAC2    |= MAC2_FULL_DUP;
